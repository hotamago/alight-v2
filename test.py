...
def main_process():
    ...
    while True:
        ...
        if cfg['on_cam1'] and camera1:
            ...
            camera1.setBrightness(cv2.getTrackbarPos(
                "Brightness1", "Camera Config"))
            camera1.setContrast(cv2.getTrackbarPos(
                "Contrast1",   "Camera Config"))
            camera1.setSaturation(cv2.getTrackbarPos(
                "Saturation1", "Camera Config"))
        if cfg['on_cam2'] and camera2:
            raw_e2 = cv2.getTrackbarPos("Exp2",        "Camera Config")
            e2 = raw_e2 / 10.0 - 10.0
            ae2 = cv2.getTrackbarPos("AutoExp2",    "Camera Config")
            camera2.setExposure(e2, ae2)
            camera2.setBrightness(cv2.getTrackbarPos(
                "Brightness2", "Camera Config"))
            camera2.setContrast(cv2.getTrackbarPos(
                "Contrast2",   "Camera Config"))
            camera2.setSaturation(cv2.getTrackbarPos(
                "Saturation2", "Camera Config"))
