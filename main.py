...
def process_mouse_click(
    cfg, mouse, raw_clicked, point, window_size, screen_size, state
):
    """
    ...
    if raw_clicked and state["old_clicked"] and not state["is_pressed"]:
        ...
        if (
            hold_time > cfg["time_delay_press"]
    ...
    if not raw_clicked and state["click_start_time"] is not None:
        hold_time = now - state["click_start_time"]
        print(
            f"[DEBUG] Mouse released, hold_time: {hold_time}, max_move: {state['max_move_dist']}"
        )
        if state["is_pressed"]:
            mouse.release(Button.left)
            print("[DEBUG] Release left")
        elif (
            hold_time > cfg["time_delay_right_click"]
            and state["max_move_dist"] <= cfg["circle_in_right_click"]
        ):
...
def main_process():
    ...
    if cfg["camera1_type"] == 0:
        ...
        if cfg["on_cam1"]:
            ...
            cv2.createTrackbar(
                "Brightness1",
                "Camera Config",
                int(cfg.get("cam1_brightness", 128)),
                255,
                lambda v: None,
            )
            cv2.createTrackbar(
            ...
            cv2.createTrackbar(
                "Saturation1",
                "Camera Config",
                int(cfg.get("cam1_saturation", 128)),
                255,
                lambda v: None,
            )
            cv2.createTrackbar(
                "Flip", "Camera Config", cfg["cam1_flip_mode"], 2, lambda v: None
            )
    elif cfg["camera1_type"] == 1:
        cv2.namedWindow("Camera Config", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Camera Config", 600, 150)
        # Range iso 100-6400
        cv2.createTrackbar(
            "ISO", "Camera Config", int(camera1.current_iso - 100), 6400, lambda v: None
        )
        # Range exposure 100000-10000000 (convert to // 10000)
        cv2.createTrackbar(
            "Exposure",
            "Camera Config",
            int(camera1.current_exposure_ns - 100000) // 10000,
            10000000 // 10000,
            lambda v: None,
        )
        cv2.createTrackbar(
    ...
    while True:
        ...
        if cfg["on_cam1"] and camera1:
            if cfg["camera1_type"] == 0 and (
                not is_detect_corners or not calibration.done
            ):
        ...
        if q == ord("s"):
            cfg["on_black_points_touch_screen"] = not cfg[
                "on_black_points_touch_screen"
            ]
        ...
        if (not is_detect_corners) or (not calibration.done):
            if not is_detect_corners_1:
                showOneBigQRcorners()
                if cfg["on_cam1"] and imgCam1 is not None:
                    is_detect_corners_1, maCam1, maCam1YXZ = get4CornersSimple(
                        imgCam1,
                        lambda x: (x[0], x[2], x[1], x[3]),
                        delta_point=tuple(cfg["delta_point_qr"]),
                    )
        ...
        if is_detect_corners_1 and calibration.done:
            ...
            imgCam1_onlyc1 = cv2.flip(
                imgCam1_onlyc1, cv2.getTrackbarPos("Flip", "Camera Config") - 1
            )
            ...
            if mode_running == 1:  # Mode lazer pen
                if cfg["camera1_type"] == 0:
                    ...
                    camera1.set_exposure_ns(
                        cv2.getTrackbarPos("Exposure", "Camera Config") * 10000 + 10000
                    )
