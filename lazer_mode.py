                ...
                """
                    ...
                    for hulls in contoursFigue_cam1:
                        ...
                        list_5_bestest_hull_point.append(
                            (int(point[0] + delta_Point[0]), int(point[1] + delta_Point[1])))
                        cnt_5_bestest_hull_point -= 1
                        if cnt_5_bestest_hull_point <= 0:
                            break
            ...
            if on_black_points_touch_screen:
                ...
                if on_paint_test:
                    for point in array_points_paint:
                        cv2.circle(
                            imgFigueDraw, point, maxRadiusFigueContour, (255, 255, 255), -1, cv2.LINE_AA)
                for point in list_5_bestest_hull_point:
                    if isClickedPoints[index_contourF]:
                        array_points_paint.append(point)
                        cv2.circle(
                            imgFigueDraw, point, maxRadiusFigueContour, color_clicked, -1, cv2.LINE_AA)
                    else:
                        cv2.circle(imgFigueDraw, point, maxRadiusFigueContour,
                                   color_nonClicked, -1, cv2.LINE_AA)
                    index_contourF += 1
