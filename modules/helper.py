...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        hsv_color = image_processor.get_hsv_pos(
            param[0], param[1], (mouse_y, mouse_x))
        print('hsv = (', hsv_color[0], ",",
              hsv_color[1], ",", hsv_color[2], ')', sep='')

        ycbcr_color = image_processor.get_ycbcr_pos(
            param[0], param[1], (mouse_y, mouse_x))
        print('ycbcr = (', ycbcr_color[0], ",",
              ycbcr_color[1], ",", ycbcr_color[2], ')', sep='')
        ...
        if len(list_4_points) >= 4:
            print('(', end='', sep='')
            for i in range(0, 4):
                print('(', list_4_points[i][0], ',',
                      list_4_points[i][1], ')', end='', sep='')
                if i < 3:
                    print(',', end='', sep='')
            print(')')
            list_4_points.clear()
        if len(list_10_hsv) >= 10:
            print('(', end='', sep='')
            for i in range(0, 10):
                print('(', list_10_hsv[i][0], ',', list_10_hsv[i]
                      [1], ',', list_10_hsv[i][2], ')', end='', sep='')
                if i < 9:
                    print(',', end='', sep='')
            print(')')
            list_10_hsv.clear()
...
def auto_ProcessImage_onlyhand(imgCam, maCamYXZ, gamma, fillCam_01, noseCam):
    ...
    imgCamFTI = matrix_bincase.fast_tranform_image_opencv(
        imgCam, maCamYXZ, size_window)
    imgFigue = image_processor.detect_hand_v2(
        imgCamFTI, gamma, fillCam_01, noseCam)
    return imgFigue
...
def auto_ProcessImage_nofti(imgCam, gamma, fillCam_01, noseCam, on_show_cam, on_camHsv, on_camYcbcr, on_camFTI, title_on_show_cam):
    ...
    contoursFigue, hierarchyFigue = cv2.findContours(
...
def auto_ProcessImage(imgCam, maCamYXZ, gamma, fillCam_01, noseCam, on_show_cam, on_camHsv, on_camYcbcr, on_camFTI, title_on_show_cam):
  ...
  """
    ...
    contoursFigue, hierarchyFigue = cv2.findContours(
        imgFigue, cv2.RETR_LIST, cv2.RETR_EXTERNAL)
...
def setFullScreenCV(nameWindow):
    ...
    cv2.setWindowProperty(
        nameWindow, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
...
def get4Corners_chess(imgCam, size_chess, lambda_format_ma, delta_point=(25, 30)):
    ...
    ret, points_beta = cv2.findChessboardCorners(
        gray, size_chess, flags=cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_FAST_CHECK + cv2.CALIB_CB_NORMALIZE_IMAGE)
    ...
    if ret == True:
        ...
        rect += np.array([[-delta_point[0], -delta_point[1]], [delta_point[0], -delta_point[1]],
                         [-delta_point[0], delta_point[1]], [delta_point[0], delta_point[1]]], dtype=np.float32)
...
def get4Corners_circle(imgCam, lambda_format_ma, minDist, param1, param2, minRadius, maxRadius, delta_point=(25, 25)):
    ...
    if (circles is not None) and circles[0].shape[0] >= 4:
        ...
        points = np.array(
            list(map(lambda point: (point[0], point[1]), points_beta)))
        ...
        rect += np.array([
            [-(delta_point[0] + points_beta[p_point[0]][2]), -
             (delta_point[1] + points_beta[p_point[0]][2])],
            [(delta_point[0] + points_beta[p_point[1]][2]), -
                (delta_point[1] + points_beta[p_point[1]][2])],
            [-(delta_point[0] + points_beta[p_point[2]][2]),
             (delta_point[1] + points_beta[p_point[2]][2])],
