...
def auto_ProcessImage_onlyhand(imgCam, maCamYXZ, gamma, fillCam_01, noseCam):
  ...
  imgCamFTI = matrix_bincase.fast_tranform_image_opencv(imgCam, maCamYXZ, cfg['size_window'])
...
def auto_ProcessImage(imgCam, maCamYXZ, gamma, fillCam_01, noseCam, on_show_cam, on_camHsv, on_camYcbcr, on_camFTI, title_on_show_cam):
  ### Process image: Perspective, filter_color, filter_noise, findContours ###
  imgCamFTI = matrix_bincase.fast_tranform_image_opencv(imgCam, maCamYXZ, tuple(cfg['size_window']))
  imgFigue = image_processor.detect_hand_v2(imgCamFTI, gamma, fillCam_01, noseCam)
...
# Auto detect corners
imgQRcorners = qr.given_image_corners_qr(tuple(cfg['fullscreensize']), cfg['core_value_qr'])
...
# Placeholder for corner calculation logic based on QR polygon points
def get_corner_from_polygon_tl(polygon): return polygon[0] # Example: Top-left vertex
def get_corner_from_polygon_tr(polygon): return polygon[1] # Example: Top-right vertex
def get_corner_from_polygon_bl(polygon): return polygon[3] # Example: Bottom-left vertex
def get_corner_from_polygon_br(polygon): return polygon[2] # Example: Bottom-right vertex

array_get_corner_qr = [
    get_corner_from_polygon_tl,
    get_corner_from_polygon_tr,
    get_corner_from_polygon_bl,
    get_corner_from_polygon_br
]
...
def get4Corners(imgCam, lambda_format_ma, delta_point = (0, 0)):
  ...
  maCam_beta = np.zeros((4, 2), dtype=np.float32)
  ...
  if len(values) > 0:
    for value in values:
      ...
      if qr_value in cfg['set_detect_value_qr']:
        idx = cfg['set_detect_value_qr'].index(qr_value)
  ...
  if is_detect_corners:
    ...
    maCam = tuple(map(tuple, maCam_beta))
    maCamYXZ = lambda_format_ma(maCam)
  return is_detect_corners, maCam, maCamYXZ

def get4Corners_chess(imgCam, size_chess, lambda_format_ma, delta_point = tuple(cfg['delta_point_chess'])):
  global imgQRcorners
  maCam = ((0, 0), (0, 0), (0, 0), (0, 0))
  maCamYXZ = ((0, 0), (0, 0), (0, 0), (0, 0))
...
def get4Corners_circle(imgCam, lambda_format_ma, minDist, param1, param2, minRadius, maxRadius, delta_point = tuple(cfg['delta_point_circle'])):
  maCam = ((0, 0), (0, 0), (0, 0), (0, 0))
  maCamYXZ = ((0, 0), (0, 0), (0, 0), (0, 0))
